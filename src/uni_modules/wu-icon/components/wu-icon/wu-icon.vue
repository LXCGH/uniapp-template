<template>
  <view class="wu-icon" @tap="clickHandler" :class="['wu-icon--' + labelPos]">
    ...
    <text v-if="label !== '' && (labelPos == 'left' || labelPos == 'top')" class="wu-icon__label" :style="labelStyle">{{
      label
    }}</text>
    <image
      class="wu-icon__img"
      ...
      :src="name"
      :mode="imgMode"
      :style="[imgStyle, $w.addStyle(customStyle)]"
    ></image>
    <text
      v-else
      class="wu-icon__icon"
      :class="uClasses"
      :style="[iconStyle, $w.addStyle(customStyle)]"
      :hover-class="hoverClass"
      >{{ icon }}</text
    >
    <!-- 这里进行空字符串判断，如果仅仅是v-if="label"，可能会出现传递0的时候，结果也无法显示 -->
    <text
      v-if="label !== '' && (labelPos == 'right' || labelPos == 'bottom')"
