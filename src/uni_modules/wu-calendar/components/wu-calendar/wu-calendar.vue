      ...
      <slot name="operation" v-if="operationPosition == 'top'">
        <view v-if="!insert" class="wu-calendar__header wu-calendar--fixed-top">
          ...
          <view class="wu-calendar__header-btn-box" @click="confirm">
            <text class="wu-calendar__header-text wu-calendar--fixed-width" :style="[{ color: confirmColor }]">{{
              okText
            }}</text>
          </view>
        </view>
  ...
  methods: {
    ...
    updateWeeks(isChange = true, isInt = false) {
      ...
      if (isChange) {
        ...
        let fullDate =
          this.FoldStatus === "close" ? this.nowDate.fullDate : `${this.nowDate.year}-${this.nowDate.month}-${1}`;
        newFullDate = this.cale.getDate(
          fullDate,
