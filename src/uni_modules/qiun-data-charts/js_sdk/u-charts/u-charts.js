...
function fixLineSeries(series, opts, config) {
  let lineSeriesArr = [];
  if(series.length>0 && series[0].data.constructor.toString().indexOf('Array') > -1){
    ...
    series[0].data = data;
    if (!series[0].labelText) {
      series[0].labelText = labelText;
    }
    lineSeriesArr.push(series[0]);
  }else{
    lineSeriesArr = series;
  }

  return lineSeriesArr;
}
...
function drawCharts(type, opts, config, context) {
  ...
  if (type === 'line') {
    series = fixLineSeries(series, opts, config);
  }
  //兼容ECharts饼图类数据格式
